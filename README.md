...
A collection of [Claude Code](https://docs.anthropic.com/en/docs/claude-code) Plugins and Skills for **PRD 全流程工作流**（PRD authoring → bi-directional sync → User Story pipeline → Release Announcement）on [Lark / 飞书](https://www.larksuite.com/).

> **作者：** 雪松-Ash Zeng · **License：** MIT

---

## 🎯 PRD 全流程（Full Pipeline）

四个技能互相独立又串联，覆盖**从功能大纲到发版公告**的完整链路：
...
| `lark-workflow-prd-to-userstory` | PRD 功能清单 → P0 User Story → 飞书需求矩阵 Base 表 → 飞书项目对应版本工作项。 | `prd to userstory` / `prd 转需求` / `按版本批量创建需求` |
| `release-announcement` | PRD → 三版发版公告（简略版 + 详细中文版 + 详细英文版），自动创建为飞书云文档。 | `生成发版公告` / `release notes` / `发版简报` |
...
| `§5` 功能清单 | `功能模块 / 描述 / P0 / 待开发 / 备注` 表格 | prd-sync 自动生成 | prd-to-userstory 解析 / release-announcement 参考 |
...
| 发版公告文档创建在 bot 名下 | 命令缺少 `--as user` | release-announcement Step 4 强制检查，见 SKILL.md |
